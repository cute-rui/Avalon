...
func CheckIsEnd(ss string) bool {
    var VideoList *bilibili.ParsedQuery
    var err error
    retry := 1
    ...
    for i := 0; i <= retry; i++ {
        VideoList, err = bilibili.DoDownloadQuery(utils.StringBuilder(`https://www.bilibili.com/bangumi/play/ss`, ss), false)
        if err == nil {
            break
        }
        log.Logger.Error(err, `on updating anime season: `, ss)
    }
